  ...
  return (
    <div className="md:col-span-6 bg-surface rounded-3xl p-8 border border-white/10 flex flex-col h-full">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-6">
        <div>
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{frontend.title}</h4>
        ...
        <div>
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{backend.title}</h4>
          <ul className="space-y-2">
        ...
        <div className="col-span-2 mt-2 pt-4 border-t border-white/10">
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{mobile.title}</h4>
          <div className="flex gap-3">
            {mobile.items.map((item, index) => (
              <span key={index} className="px-3 py-1.5 bg-surface-light rounded-lg text-xs text-white border border-white/10">{item}</span>
            ))}
          </div>
        </div>
      </div>
