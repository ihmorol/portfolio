...
export const Certifications = () => {
  ...
  return (
    <div className="md:col-span-7 bg-surface rounded-3xl p-6 md:px-8 md:py-8 border border-border">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold text-text-primary">Certifications</h3>
        <a className="text-xs text-text-secondary hover:text-text-primary transition-colors cursor-pointer">View Credentials</a>
      </div>
      <div className="space-y-4">
        {certifications.map((cert, index) => (
          <div key={index} className="group flex items-start gap-4 p-3 rounded-2xl hover:bg-surface-light transition-colors cursor-pointer">
            ...
            <div>
              <h4 className={`text-sm font-bold text-text-primary ${cert.hoverColor} transition-colors`}>{cert.title}</h4>
              <p className="text-[11px] text-text-secondary">{cert.issuer}</p>
            </div>
          </div>
        ))}
      </div>
