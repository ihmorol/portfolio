...
export function SocialChannels() {
  ...
  const getColorClasses = (colorIdx: string) => {
      switch(colorIdx) {
          case 'gray': return 'text-text-secondary hover:bg-foreground hover:text-background';
          ...
          default: return 'text-text-primary hover:bg-foreground';
      }
  };

  return (
    <section className="glass-dark rounded-3xl p-8 mb-16 border border-border/60 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex flex-wrap justify-center gap-6">
            {channels.map((channel) => (
                <a
                    key={channel.name}
                    className={`focus-ring h-12 w-12 rounded-full bg-background-tertiary border border-border/60 flex items-center justify-center transition-all duration-300 glow-hover hover:text-text-primary ${getColorClasses(channel.colorIdx)}`}
                    href={channel.url}
                    title={channel.name}
                    aria-label={channel.name}
                    {...(channel.url.startsWith('http') ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
                >
                    <span aria-hidden="true" className="material-symbols-outlined text-2xl">{channel.icon}</span>
                </a>
            ))}
        </div>
    </section>
