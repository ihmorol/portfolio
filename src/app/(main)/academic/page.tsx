...
export default function AcademicPage() {
  ...
  return (
    <div className="bg-background text-text-primary font-display overflow-x-hidden min-h-screen flex flex-col selection:bg-foreground selection:text-background">
      <main className="flex-grow flex flex-col items-center w-full max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12 pt-20 md:pt-24 pb-8 md:pb-12 relative">
        ...
        <PageHeader
          ...
          description="My degree, thesis, coursework, and awards at United International University."
        />
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 pb-12 md:pb-16 max-w-6xl mx-auto">
          ...
          {/* Current Degree Card */}
          <div className="md:col-span-12 bg-background-secondary border border-border rounded-3xl p-2 md:p-3 flex flex-col md:flex-row gap-6 md:gap-12 overflow-hidden min-h-[350px]">
            <div className="p-6 md:p-10 md:pr-0 flex flex-col justify-center flex-1">
              <div className="flex items-center gap-2 mb-2">
                <span className="bg-foreground text-background text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">{currentDegree.badge}</span>
              </div>
              <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-2">{currentDegree.degree}</h2>
              <h3 className="text-xl md:text-2xl font-semibold text-text-secondary mb-1">{currentDegree.university}</h3>
              <p className="text-text-muted text-sm font-medium mb-4 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <p className="text-xs text-text-muted uppercase font-bold tracking-wider">GPA</p>
                  <p className="text-text-primary font-bold text-lg">{currentDegree.gpa}</p>
                </div>
                <div>
                  <p className="text-xs text-text-muted uppercase font-bold tracking-wider">Program Focus</p>
                  <p className="text-text-primary font-bold text-lg">{currentDegree.focus}</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {currentDegree.awards.map((award) => (
                    <span key={award} className="bg-background-tertiary text-text-secondary text-xs px-3 py-1 rounded-full font-medium border border-border">{award}</span>
                ))}
              </div>
            </div>
            
            {/* Visual Illustration */}
            <div className="bg-background-tertiary rounded-2xl flex-1 relative min-h-[300px] md:min-h-auto overflow-hidden flex items-center justify-center p-8">
              <div className="relative w-full h-full flex flex-col items-center justify-center gap-6">
                ...
                <div className="bg-background-secondary px-6 py-3 rounded-full shadow-lg border border-border flex items-center gap-3 z-10">
                  <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
                  <span className="text-text-primary font-bold text-sm">{currentDegree.visualLabel}</span>
                </div>
            ...
            <div className="flex-grow p-6 md:p-8">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl md:text-2xl font-bold text-text-primary">Undergraduate Thesis</h3>
                </div>
              ...
              <h4 className="text-lg font-bold text-text-primary mb-2">{thesis.title}</h4>
              <p className="text-sm text-text-secondary mb-6 font-body leading-relaxed">
              ...
              <div className="bg-surface-light rounded-xl p-5 mb-6 border border-border/50">
                <p className="text-sm text-text-secondary italic mb-2">{thesis.quote}</p>
                <div className="flex flex-wrap gap-2 mt-3">
                  {thesis.techStack.map((tech) => (
                      <span key={tech} className="text-[10px] bg-foreground/10 text-text-secondary px-2 py-1 rounded border border-border">{tech}</span>
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h5 className="text-xs font-bold text-text-muted uppercase mb-1">Methodology</h5>
                  <p className="text-xs text-text-secondary">{thesis.methodology}</p>
                </div>
                <div>
                  <h5 className="text-xs font-bold text-text-muted uppercase mb-1">Expected Outcomes</h5>
                  <p className="text-xs text-text-secondary">{thesis.outcomes}</p>
                </div>
              </div>
            </div>
            <div className="p-6 pt-0 mt-auto">
              <Link href={thesis.proposalLink} className="flex items-center justify-center gap-2 w-full bg-foreground hover:bg-foreground/85 text-background text-xs font-bold py-3 px-4 rounded-full transition-all uppercase tracking-wider text-center">
                ...
                {certifications.map((cert, index) => (
                    <div key={index} className="group/item border-b border-border pb-4 last:border-0">
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="text-sm font-bold text-text-primary group-hover/item:text-blue-400 transition-colors">{cert.title}</h4>
                        <span className="text-[10px] text-text-muted">{cert.year}</span>
                      </div>
                      <p className="text-xs text-text-secondary mb-1">{cert.issuer}</p>
                      <p className="text-[10px] text-text-muted">{cert.skills}</p>
                    </div>
                ))}
              </div>
              <button className="w-full bg-surface-light hover:bg-border text-text-primary text-[10px] font-bold py-3 rounded-full transition-all uppercase tracking-wider mt-6 border border-border">
          ...
          <div className="md:col-span-12 bg-surface rounded-3xl p-6 md:p-10 border border-border flex flex-col">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h3 className="text-2xl font-bold text-text-primary leading-tight">Relevant Coursework</h3>
                <p className="text-xs text-text-secondary mt-1 font-body">Detailed breakdown of academic focus areas</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {coursework.map((category, index) => (
                  <div key={index} className={`bg-surface-light/30 rounded-2xl p-5 border border-border/60 hover:border-border transition-colors ${category.colSpan || ''}`}>
                    <h4 className={`text-sm font-bold ${category.colorClass} uppercase tracking-wider mb-4 border-b border-border/60 pb-2`}>{category.category}</h4>
                    {category.subLists ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {category.subLists.map((subList, idx) => (
                                <ul key={idx} className="space-y-2">
                                    {subList.map((course) => (
                                        <li key={course} className="flex items-start gap-2 text-xs text-text-secondary">
                                            <span className="material-symbols-outlined text-[14px] text-text-muted mt-0.5">check_circle</span>
                                            <span>{course}</span>
                                        </li>
                                    ))}
                                </ul>
          ...
          <div className="md:col-span-12 grid grid-cols-1 md:grid-cols-12 gap-6">
            <div className="md:col-span-7 bg-surface rounded-3xl p-6 md:px-8 md:py-8 border border-border flex flex-col">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-lg font-bold text-text-primary">Workshops & Seminars</h3>
              </div>
              <div className="space-y-4">
                {workshops.map((workshop, index) => (
                    <div key={index} className="flex gap-4 items-start">
                      <div className="flex flex-col items-center">
                        <span className="text-[10px] font-bold text-text-muted">{workshop.month}</span>
                        <span className="text-lg font-bold text-text-primary">{workshop.day}</span>
                      </div>
                      <div className={`flex-1 ${index < workshops.length - 1 ? 'pb-4 border-b border-border/50' : ''}`}>
                        <h4 className="text-sm font-bold text-text-primary">{workshop.title}</h4>
                        <p className="text-xs text-text-secondary">{workshop.description}</p>
                      </div>
                    </div>
                ))}
              </div>
            ...
            <div className="md:col-span-5 bg-gradient-to-br from-surface to-surface-light rounded-3xl p-6 md:px-8 md:py-8 border border-border flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-3">
                  {achievements.map((item, index) => (
                      <div key={index} className="flex items-center gap-3 p-3 bg-black/20 rounded-xl border border-border/60">
              ...
              <button className="mt-6 text-[10px] text-text-secondary hover:text-text-primary transition-colors flex items-center gap-1 w-full justify-center">
